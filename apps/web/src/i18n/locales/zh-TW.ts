...
export const zhTW: Dict = {
  ...
  'chat.amrCard.switchBody': '可切換到 Open Docs 官方 AMR 模型服務，無需設定 API Key。完成登入、授權與儲值後，將自動重試目前任務。',
  ...
  'chat.runError.signInMessage.amr': 'Open Docs 智慧體尚未登入，前往登入即可正常使用',
  'chat.runError.signInMessage.other': '{agent}尚未登入，請在本機檢查登入狀態。推薦使用 Open Docs 智慧體，更穩定划算',
  ...
  'settings.welcomeTitle': '歡迎使用 Open Docs',
  'settings.welcomeSubtitle': '',
  'settings.onboardingCreateTitle': '從一句需求開始',
  'settings.onboardingCreateBody': '描述你想做的網站、應用、Deck、圖片或影片，Open Docs 會建立專案，並保留可繼續編輯的產出。',
  ...
  'settings.onboardingMemoryCalloutBody': '這些回答會構成你的記憶檔案。Open Docs 會在每個任務中重複使用它，並在你工作時持續學習。',
  ...
  'settings.onboardingCloudTitle': '登入 Open Docs',
  'settings.onboardingCloudBody': '登入後可直接用雲端 AI 開始設計，無需任何複雜設定。',
  'settings.onboardingCloudSignIn': '登入 Open Docs 雲端',
  ...
  'settings.onboardingGateTooltipAmr': '後續流程由 AI 執行，請先登入 Open Docs AMR。',
  ...
  'settings.agentInstall.pathHint': '如果你透過 npm 或 Homebrew 安裝了 CLI，但仍顯示為未安裝，請確認該工具的 bin 目錄已加入 Open Docs daemon 繼承的 PATH（在 macOS 上，Terminal 與 GUI 應用的 PATH 可能不同）。請參閱 QUICKSTART.md（「Local agent CLI and PATH」章節）。',
  'settings.agentInstall.stepOpenLinks': '在目標代理卡片上開啟「安裝」或「文件」連結。',
  'settings.agentInstall.stepAuth': '返回 Open Docs 前，請先在對應 CLI 完成驗證（登入或新增 API 憑證）。',
  ...
  'settings.mcpServerHint': '將 Open Docs 作為 MCP 伺服器暴露給你的編碼代理。',
  ...
  'settings.mcpHint': '讓其他專案中的程式碼代理（Claude Code、Cursor、VS Code、Antigravity、Zed、Windsurf）讀取您的 Open Docs 專案。您可以直接將設計匯入應用程式，無需先匯出 zip。',
  'settings.mcpDaemonError': '無法連線到本地守護程序以解析安裝路徑（{error}）。請確認 Open Docs 正在執行，然後重新開啟此面板。',
  ...
  'settings.mcpCapabilityDefault': '預設使用您在 Open Docs 中開啟的專案與檔案，因此您可以說「在我的應用程式中建置這個」，無需重新說明是哪個設計。',
  'settings.mcpRunningNote': 'Open Docs 必須正在執行，MCP 工具呼叫才能成功。如果您在開啟 Open Docs 之前就已啟動 coding agent，請重新啟動 agent，使其能夠連線到正在執行的守護行程。',
  ...
  'homeHero.subtitlePrefix': '由 YoungJin Cho 在 LGCNS 交付',
  ...
  'homeHero.chip.createPluginHint': '編寫可重複使用的 Open Docs 外掛並將其加入「我的外掛」。',
  ...
  'homeWorkingDir.hint': '讓 Agent 可讀取該本機目錄（不會匯入 Docs Files）',
  ...
  'handoff.promptIntro': '請基於這個 Open Docs 專案的本機資料夾繼續實作：',
  ...
  'pluginsHome.subtitle': '隨附於此執行環境、可立即使用的 Open Docs 工作流程。挑選一個以載入起始提示，或瀏覽登錄檔取得更多。',
  'pluginsHome.browseRegistry': '瀏覽登錄檔',
  'pluginsHome.count': '{total} 個中的 {filtered} 個',
  'pluginsHome.loadingCatalog': '正在載入目錄…',
  'pluginsHome.emptyCatalog': '目錄是空的。內建外掛隨 Open Docs 一同提供，應會自動出現在此處 — 若問題持續發生，請嘗試重新啟動 daemon。',
  ...
  'pluginCard.contributeAria': '將 {title} 貢獻至 Open Docs',
  'pluginCard.contributeTitle': '透過 pull request 將外掛貢獻至 Open Docs',
  ...
  'integrations.lede': '連接外部系統，把 MCP 工具帶入智能體迴圈，並在其他 IDE、腳本與自動化流程中使用 Open Docs。',
  ...
  'mcpClient.daemonError': '無法連線到本機 daemon。請確認 Open Docs 正在執行，然後重新開啟此面板。',
  'mcpClient.saveFailed': '儲存失敗。請確認 daemon 正在執行後再試一次。',
  'useEverywhere.modalAria': '隨處使用 Open Docs',
  'useEverywhere.modalTitle': '隨處使用 Open Docs',
  'useEverywhere.modalSubtitle': '將 Open Docs 接入任何 IDE、Agent 或指令碼 — CLI、HTTP、MCP 及 Skills。使用「複製 Agent 指南」並貼到 Claude Code、Codex、Cursor、openclaw 或 hermes 中，即可設定一切。',
  ...
  'useEverywhere.section.overview.heading': '你的 Agent 在哪裡運作，Open Docs 就在哪裡運作',
  'useEverywhere.section.overview.intro': 'Open Docs 不只是一個視窗 — 它是一個本機特權 daemon（`od`），外加一個 Skills + Design-Systems + Atoms 登錄。一旦它在您的機器上執行，任何程式碼 Agent（Claude Code、Codex、Cursor、OpenCode/openclaw、Hermes、您自己的指令碼）都能透過四個可互換的介面來驅動產生作業、檢視專案並產出設計成品。',
  'useEverywhere.section.overview.bullet1': 'CLI — 以 `od <command>` 用於無頭指令碼、CI 及 shell 自動化。',
  'useEverywhere.section.overview.bullet2': 'MCP server — 將 Open Docs 接成 Model Context Protocol 伺服器，讓任何支援 MCP 的 Agent 都能列出技能、執行情境並讀取成品。',
  'useEverywhere.section.overview.bullet3': 'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE 端點；與網頁 UI 使用的介面相同。',
  'useEverywhere.section.overview.bullet4': 'Skills — 隨插即用的 `SKILL.md` 套件（相容於 Claude），任何已在您 PATH 上的 Agent 完全無需 Open Docs 即可調用。',
  ...
  'useEverywhere.section.cli.heading': '從任何 shell 操作 Open Docs',
  ...
  'useEverywhere.section.mcp.heading': '將 Open Docs 以 MCP 伺服器形式開放給任何程式設計 Agent',
  'useEverywhere.section.mcp.intro': 'Open Docs 隨附一個 Model Context Protocol 伺服器（`od mcp`），讓任何支援 MCP 的用戶端 — Cursor、Claude Code、Antigravity、VS Code Copilot Chat、openclaw、hermes — 都能探索 Open Docs 工具（列出技能、算繪預覽、產生媒體、執行外掛），而無需手動使用 shell。daemon 透過 `GET /api/mcp/install-info` 為每個主要用戶端發布可直接貼上的安裝程式碼片段。',
  ...
  'useEverywhere.section.mcp.footer': '在 Open Docs 應用程式中，開啟「設定 → 整合」即可複製特定用戶端的安裝指令（Cursor、Claude Code、Antigravity、VS Code），而無需手動編輯 JSON。',
  ...
  'useEverywhere.section.skills.heading': '適用於任何 Agent 的隨插即用 Skills — 即使 Open Docs 未在執行也行',
  'useEverywhere.section.skills.intro': 'Skill 是一個包含相容於 Claude 的 `SKILL.md`（YAML 前置資料 + 內文）的目錄。Open Docs 以 `od:` 命名空間（`mode`、`preview`、`design_system`、`inputs`、…）擴充此格式，因此同一個成品既能在 Open Docs 內使用，也能被像 Claude Code、Codex、openclaw 或 hermes 這類原生 Agent 使用。探索作業遵循一條優先順序鏈，因此專案可覆寫自己的技能。',
  ...
  'useEverywhere.section.skills.snippet1': '最精簡的 SKILL.md（Claude 相容的前置資料 + Open Docs 額外設定）',
  ...
  'entry.githubStarAria': '在 GitHub 為 Open Docs 點亮 Star',
  ...
  'newproj.locationDefault': 'Open Docs 專案',
  ...
  'socialShare.openDesignSection': '分享 Open Docs',
  'socialShare.openDesignTitle': 'GitHub 上的 Open Docs',
  'socialShare.openDesignText': 'Open Docs 是開源的設計智慧工作區，可以生成、編輯、部署並交付設計產物。',
  ...
  'socialShare.projectText': '我用 Open Docs 做了 {title}。查看已部署產物，也推薦收藏倉庫：{repo}',
  'socialShare.projectCopyText': '我用 Open Docs 做了 {title}\n{url}\nOpen Docs 倉庫：{repo}',
  ...
  'chat.brandBrowserAssistDesktopOnly': '瀏覽器輔助需要桌面應用，Open Docs 才能讀取即時瀏覽器頁面。',
  'chat.brandBrowserAssistReadFailed': '無法讀取瀏覽器頁面。請確認品牌網站已開啟並完整載入。',
  'chat.brandBrowserAssistMessage': 'Open Docs 需要瀏覽器輔助才能完成這次品牌擷取。請先在 Browser 分頁清除頁面攔截，然後在下方確認。',
  'brandEnrichment.title': "AI 優化",
  'brandEnrichment.body': "Open Docs 會在背景執行更深度的抽取，自動套用合適的設計系統 skills，並就地更新這個系統。通常需要 10-20 分鐘，你可以繼續做其他事。",
  ...
  'chat.activeFilePlaceholder': '讓 Open Docs 修改 {file}...',
  ...
  'chat.mode.design.solves': '當你希望 Open Docs 產生檔案、更新 workspace，或把需求變成多模態可交付結果時使用。',
  ...
  'chat.designToolbox.searchPlaceholder': '搜索 skills / MCP / plugins / connectors / docs files…',
  ...
  'chat.designToolbox.prompt.noSkill': '如果没有匹配到具体 skill，请先从当前可用 skills / MCP / plugins / connectors / docs files 中选择最合适的组合。',
  ...
  'chat.designToolbox.prompt.autoMatchStep2': '2. 搜索并匹配上面的全量资源索引，不要只看设计百宝箱推荐项；需要时组合 skills、MCP、plugins、connectors 和 docs files。',
  ...
  'chat.designToolbox.prompt.fileResource': '请把这个 docs file 作为当前设计对象或参考资产，基于它完成下一步加工；如果需要同时比较其他文件，请从全量 docs files 中继续搜索。',
  'chat.designToolbox.prompt.resourceIndex': '全局资源索引：skills({skills})、plugins({plugins})、MCP({mcpEnabled} enabled / {mcpTemplates} templates)、connectors({connectors} connected)、docs files({files})。',
  ...
  'chat.designToolbox.prompt.referenceDesignFiles': '可参考 docs files',
  ...
  'preview.shareTextDefault': 'Open Docs 模板：{title}',
  ...
  'preview.errorBody': '範例 HTML 載入失敗。請確認 Open Docs 正在執行後重試。',
  'preview.retry': '重試',
  ...
  'assistant.shareToOpenDocs': '分享到 Open Docs',
  'assistant.shareToOpenDocsBusy': '正在準備投稿…',
  ...
  'pet.navHint': '領養與自訂',
  'pet.tabBuiltIn': '內建',
  'pet.tabBuiltInHint': 'Open Docs 內建的精選寵物 — 一鍵領養。',
  ...
  'pet.codexBundled': '內建',
  'pet.codexBundledTitle': 'Open Docs 內建寵物，無需下載。',
  ...
  'settings.projectLocationsDescription': '新增可包含多個 Open Docs 專案資料夾的工作基目錄。新專案會儲存為所選基目錄中的一個資料夾。',
  ...
  'settings.customInstructionsHint': 'Open Docs 在每次聊天中都應遵循的固定指示。將事實、偏好與專案脈絡儲存為記憶。',
  ...
  'settings.memoryStarterProjectDesc': 'Open Docs v0.5 — 聊天驅動的設計編輯器',
  ...
  'artifact.odCardBrandAssistBody': '在瀏覽器中開啟品牌網站，讓 Open Docs 繼續擷取此設計體系。',
  ...
  'updater.availableBody': 'Open Docs {version} 可用。下載完成後即可開啟安裝器。',
  ...
  'updater.done': '完成',
  'updater.failed': '更新失敗',
  'updater.installerOpenBody': '安裝器已開啟。Open Docs 正在結束，以便你完成更新。',
  ...
  'updater.quitButton': '結束 Open Docs',
  'updater.quitFailedBody': '安裝器已開啟，但 Open Docs 無法結束。替換應用程式前請先結束 Open Docs。',
  'updater.quitFailedTitle': '無法結束',
  'updater.quitting': '正在結束…',
  'updater.ready': '更新已就緒',
  'updater.readyGeneric': '新版本已就緒。Open Docs 會關閉並開啟安裝器。',
  'updater.readyVersion': 'Open Docs {version} 已就緒。Open Docs 會關閉並開啟安裝器。',
  ...
  'dsManager.createBody': '把你的品牌、產品、程式碼、素材與設計參考教給 Open Docs。',
  ...
  'settings.customInstructionsDesc': 'Open Docs 在每次對話中都會遵循的固定指令。它們不是儲存的記憶；事實、偏好與專案脈絡請使用「記憶」。',
  ...
  'brand.librarySubtitle': '把你的品牌套件教給 Open Docs——從網站擷取一個品牌套件，然後在任意對話中套用它。',
  ...
  'onboarding.buildBody': 'Open Docs 將你的品牌沉澱為可重用的設計系統,讓你之後打造的一切都保持品牌一致。',
  ...
  'dsCreate.sourceSectionBody': '從 GitHub repo、網站、DESIGN.md 或能體現風格的檔案開始。Open Docs 會先快速建立一套可用系統，之後 AI 可在專案中繼續細化。',
  ...
  'ds.extractionRunningBody': 'Open Docs 正在就地更新此體系。',
  ...
  'ds.sectionRunningReadingContext': 'Open Docs 正在讀取 {title} 的段落上下文。',
  ...
  'ds.repoConnectBannerBody': '連接 GitHub 後，Open Docs 可以讀取你的儲存庫，並把顏色、字體和元件擷取到此設計體系中。',
