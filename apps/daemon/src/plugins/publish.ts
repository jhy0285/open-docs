...
//   - open-design            → jhy0285/open-docs (plugins/community/<plugin-name>/).
...
export function buildPublishLink(args: {
  ...
  switch (args.catalog) {
    ...
    case 'open-design': {
      ...
      const url = newIssueUrl('jhy0285/open-docs', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'jhy0285/open-docs',
        url,
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `open-design.json` sidecar (additive Open Docs metadata).');
  lines.push('');
